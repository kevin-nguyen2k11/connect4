...
class GameStorage(tf.keras.utils.Sequence):
    ...
    #     game=self.buffer[index][0]
    #     board=self.state_from_hist(game,self.moves_per_game[index]-1)
    #     print(np.array2string(np.flip(board[:,:,0]+board[:,:,1]*2,0)))
    #     print(self.buffer[index][2])

    def save_games(self,games):
        if len(games)>self.max_saved_games:
            games=games[len(games)-self.max_saved_games:]
        write_index=(self.pointer+len(games))
        if temp:=write_index-self.max_saved_games>=0:
            self.buffer[self.pointer:]=games[:-temp]
            self.buffer[:temp]=games[-temp:]
        else:
            self.buffer[self.pointer:write_index]=games
        self.num_saved_games=self.num_saved_games+len(games)
        if self.num_saved_games>self.max_saved_games:
            self.num_saved_games=self.max_saved_games
        self.update_moves_per_game(self.pointer,write_index)
        self.pointer=write_index

    def update_moves_per_game(self,start,end):
        endpoints=[(start,self.max_saved_games),(0,end)] if end<=start else [(start,end)]
        for left,right in endpoints:
            for i in range(left,right):
                try:
                    self.moves_per_game[i]=len(self.buffer[i][0])
    ...
    def __getitem__(self,idx):
        ...
        states=np.zeros((self.batch_size,self.height,self.width,2),dtype=int)
        policies=np.zeros((self.batch_size,self.width))
        values=np.zeros((self.batch_size,1),dtype=int)
    ...
    def load_buffer(self):
        ...
        for game in self.buffer:
            if game is not None:
                break
            none_count+=1
        self.num_saved_games=self.max_saved_games-none_count
        self.update_moves_per_game(none_count,self.max_saved_games)
